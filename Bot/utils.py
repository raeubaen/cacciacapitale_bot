...
#Create summary of info about a member
def info_summary(chat_id=None, hunter=None):
    lines = []
...
def update_team_txt():
    ...
    with open("data/teams.txt", "w", encoding="utf-8") as out_file:
        ...
        for cap in cap_list:
          ...
          hunter_anag_list = [anag(hunter) for hunter in hunter_list]
          teams_summary.append(f"  {cap.anagraphic}:\n" + "    ".join(hunter_anag_list))
        out_file.write("\n\n".join(teams_summary))
